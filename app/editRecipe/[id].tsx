...
const editRecipe = () => {
    ...
    useEffect(() =>{
        if(!loading && recipe){
            ...
            setRating(recipe.stars);
            setIngredients(recipe.ingredients);
            setImageUri(recipe.imageUrl);
            setDescription(recipe.description)
            setDefaultImageUri(recipe.imageUrl);
    ...
    const saveRecipe = () => {
        ...
        updateRecipe(changedRecipe, defaultImageUri);
        setSuccess(true);
    ...
    return(
      ...
      <KeyboardAwareScrollView style={{flex: 1}}>
        <View style={styles.imagePlaceholder}>
          {imageUri ? (
            <Image source={imageUri.includes("assets") ? assetsMap[imageUri]: {uri: imageUri}} style={styles.imagePreview} />
          ) : (
            <MaterialIcons name="image" size={80} color="#a0a0a0" />
          )}
          { imageUri === defaultImageUri ?
          (<TouchableOpacity style={styles.addPhotoButton} onPress={pickImage}>
            <Text style={styles.addPhotoButtonText}>Zmień zdjęcie</Text>
          </TouchableOpacity>)
            : 
            (<>
                <TouchableOpacity style={styles.addPhotoButton2} onPress={pickImage}>
                    <Text style={styles.addPhotoButtonText}>Zmień zdjęcie</Text>
                </TouchableOpacity>
                <TouchableOpacity style={styles.deletePhotoButton} onPress={() => setImageUri(defaultImageUri)}>
                    <Text style={styles.addPhotoButtonText}>Usuń zdjęcie</Text>
                </TouchableOpacity>
            </>)
            }
        ...
        <View style={styles.formContainer}>
          ...
          <TouchableOpacity style={styles.saveButton} onPress={() => setWantEdit(true)} >
            <Text style={styles.saveButtonText}>Zapisz</Text>
          </TouchableOpacity>
        </View>
      </KeyboardAwareScrollView>
        <Modal visible={wantEdit} transparent animationType="fade">
            <TouchableOpacity style={styles.overlay} onPress={() => setWantEdit(false)}>
            <View style={styles.popup}>
                <Text style={styles.popupText}>Czy napewno chcesz edytować?</Text>
    ...
    <Modal visible={loadingUpdate} transparent animationType="fade">
            <View style={styles.overlay}>
              <ActivityIndicator size="large" color="#fff" />
            </View>
          </Modal>
          <Modal visible={success && !loadingUpdate} transparent animationType="fade">
          <View style={styles.overlay}>
            <View style={styles.popup}>
              <Text style={styles.popupText}>Przepis został zedytowany</Text>
              <TouchableOpacity
                style={styles.button}
                onPress={() => router.back()}
              >
